...
class GameTree(List):
    ...
    def __str__(self):
        ...
        if len(self):
            ...
            # accounts for line breaks within Nodes
...
class Node(Dictionary):
    ...
    def __str__(self):
        ...
        if len(self):
            s = ";" + str(self[0])
            # accounts for line breaks within Properties
            l = len(string.split(s, "\n")[-1])
...
class Cursor:
    ...
    def _setFlags(self):
        ...
        self.atEnd = not self.gametree.variations and (
            self.index + 1 == len(self.gametree))
...
def _escapeText(text):
    ...
    while match:
        output = output + text[index:match.start()] + \
            '\\' + text[match.start()]
        index = match.end()
        match = reCharsToEscape.search(text, index)
...
HA[3]AB[pd][dp][dd];W[pp];B[nq];W[oq]C[ x started observation.
    ...
    c = col.cursor()
    while 1:
        print ("nodenum: %s; index: %s; children: %s; node: %s" %
               (c.nodenum, c.index, len(c.children), c.node))
        if c.atEnd:
            break
        c.next()
